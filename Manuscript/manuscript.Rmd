...
SNL constitutes habitat for mobile organisms, and can act as sources of ecosystem services such as pollination or pest control [@albrecht2020; @gardner2021], but this can vary depending on the crop system [@gray2014; @quinn2017] and landscape context [@marja2019; @boetzl2020].
...
```{r distSmooths, echo=FALSE, fig.cap='Distance smoothers from a single field, showing a negative effect of distance on average yield (first row). The variability smoothers show an intermediate reduction in variability at around 90 m.', out.width='100%'}  
...
Overall, both yield average and yield variability were extremely spatially dependent across all fields (average p-values both <0.0001), meaning that within-field variability was the primary driver of yield across canola, wheat, and pea crops (Table \@ref(tab:smoothSummary), row 6).
...
canolaMods %>% group_by(Smoother) %>% 
  summarize(N=n(),mean=mean(pval),propSig=sum(pval<(0.05/N))/n()) %>% 
  ungroup() %>% 
  ...
  select(-N,-Smoother) %>% relocate(type,par,mean,propSig) %>% 
  pivot_wider(id_cols=type,names_from=par,values_from=c(mean,propSig)) %>% 
  relocate(type,mean_Mean,propSig_Mean,mean_Variability,propSig_Variability) %>% 
  mutate(type=c('Bare','Grassland','Other Crop','Shelterbelt','Standard','Wetland','Spatial Smoother')) %>% 
  kable(.,format='latex',caption='Mean p-values and proportion of p-values less than 0.05 (after Bonneferoni correction) for mean and variability smoothers across all fields.',col.names = c('Smoother Type','Average p-value','Proportion <0.05','Average p-value','Proportion <0.05'),
        digits = c(1,3,3,3,3),escape = FALSE) %>% 
  add_header_above(c('', 'Mean' = 2, 'Variability' = 2))
...
Pea crops were less well-represented in our data (N = `r sum(with(datSource,use&modelComplete2&crop=='Peas'))` fields, compared to ), and the effects of boundaries tended to be less consistent. 
