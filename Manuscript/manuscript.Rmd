...
fieldAreas <- sapply(paste0('.',datSource$boundaryPath),function(x){
  require(sf)
  read_sf(x) %>% st_area() %>% sum()/10000 #Field areas in hectares
})
...
<!-- Potential titles (best to worst): -->
<!-- "Livin' on the edge: Precision yield data shows evidence of ecosystem services from field boundaries" -->
<!-- "Field boundaries and ecosystem service spillover: evidence from precision yield data" -->
<!-- "Living on the edge: field boundary effects on precision yield" -->
<!-- "Ecosystem services for crop yields: do field boundaries matter?" -->
<!-- "Crop boundary effects on precision yield data" -->
...
Maintaining both biodiversity and production in agroecosystems represents a goal of conservationists and agronomists, and holds the potential for win-win scenarios ["multifunctionality" _sensu_ @mitchell2014; @frei2018; but see also @karp2018].
...
Alternatively, yield may be higher near field boundaries (Scenario 3) if boundaries provide helpful ecosystem services [@duelli2003]. 
For example, crop pollination services from bees that nest in field boundaries drops rapidly with distance [@garibaldi2011] and wind speed is reduced directly downwind of treelines [@deJong1995], so positive effects will be concentrated close to the boundary.
...
Yield may be maximized at intermediate distances (Scenario 4), suggesting a "Goldilocks" field size where negative edge effects are canceled out by ecosystem services [@kowalchuk1995; @helmers2005; @mitchell2014; @vanVooren2017].
...
This means that studies of SNL effects on crop yield tend to suffer from limited spatial scope and small sample sizes, reducing statistical power and generality [@brandle1984; @baker2018; but see also @redhead2020].
Meta-analyses are a common solution to this problem [e.g. @albrecht2020; @lowe2021], but precision yield data holds enormous promise because of its relatively common usage and low operating costs [@steel2017; @osorio2018]. 
...
To our knowledge, only [@osorio2018] has studied the effects of field boundaries using precision yield data, making this study one of the first of its kind.
...
Individual fields ranged from `r round(min(fieldAreas),1)` to `r round(max(fieldAreas),1)` ha in area (mean: `r round(mean(fieldAreas),1)` ha), and contained between `r min(yearsPerField)` and `r max(yearsPerField)` years of data (mean: `r round(mean(yearsPerField),1)`), for a total of `r round(sum(datSourceFinal$npoints)/1e6,1)` million data points.
...
We fit the following model to each field-year of data:
  ...
  4. f(x,b) = penalized thin-plate regression spline, where _x_ is the predictor and _b_ is the number of basis dimensions
...
This does not account for uncertainty in the field-level smoothers, so we used an approach similar to bootstrapping:
  ...
  3. Fit new model of simulated smoothers from all fields (with a field-year random intercept), and save this "meta-smoother".
...
```{r spatialSmooths, echo=FALSE, fig.cap="Raw data and spatial smoothers from a single field. Yield averages (means) and variability (log SD) were modeled separately, and both show large spatial dependence within the field. Field dimensions are approximately 800 x 800 m, but coordinates are hidden to protect data privacy.", out.width='100%'}  
...
```{r boundaryTypes, echo=FALSE, fig.cap='Boxplot of field boundary compositions across sampled fields. The x-axis indicates the boundary type, while the y-axis indicates the percentage of all field boundaries covered by that boundary type.', out.width='100%'}
...
Pea crops were less well-represented in our data (N = `r sum(with(datSource,use&modelComplete2&crop=='Peas'))` fields), and therefore the effects of boundaries tended to be less consistent. 
...
Shelterbelts can act to capture snow and moderate downwind microclimate, reducing water stress on crop plants and increasing yield [@staple1955; @kowalchuk1995; @bao2012]. 
However, @kowalchuk1995 found that positive microclimate effects of shelterbelts on wheat yield occur at around 20 m (3-4 times the shelterbelt height), while our work shows a peak at around 50 m.
This additionally complicated by the fact that water stress varies from year-to-year, and that the impact of the shelterbelt also depends on the orientation to prevailing winds and the physical structure of the shelterbelt [@pelton1967; @kort1988; @singh1999].
...
While we consider microclimate to be the most likely cause of the intermediate increase, it may also be related to soil effects from shelterbelts, where erosion is reduced and leaf litter from the shelterbelt can cause an increase in soil deposition [@deJong1995].
Finally, shelterbelts may also act as sources of predators or parasitoids of wheat crop pests [@dong2015; @ward2021] but their effect depends on pest pressures, which are typically suppressed by pesticide application.
...
<!-- While we found an intermediate positive effect of shelterbelts on wheat, other studies have shown variable effects, ranging from no effect [@akbar1990; @crawford1998]  -->
<!-- Our work also did not examine orientation-specific effects driven by prevailing wind (e.g. east-west distance rather than absolute distance) -->
...
Our work examined only the first and last links in the causal chain _Field Boundary_ $\rightarrow$ _Ecosystem Service_ $\rightarrow$ _Crop Yield_, meaning that are two mediating relationships where the overall relationship could be broken or obscured.
...
Our work suggests that we need to better examine the context in which a) beneficial organisms spillover from field boundaries and b) under what conditions field boundaries provide tangible ecosystem services [@karp2018].
...
For example, larger or more functionally diverse [@bonifacio2011; @boughey2011; @lecq2018; @stasiov2020] hedgerows tend to support more biodiversity than smaller hedgerows, and some studies have shown that leaf litter and soil compaction influences predatory beetle communities [@sutter2018b].
...
Finally, the relevant spatial scale of boundary effects are often related to the height or windward orientation of the shelterbelt [@kort1988; @kowalchuk1995], and precision yield data has the potential to show this in far greater detail than previous studies.
...
Ecosystem services from field boundaries may also be limited if beneficial organisms stay in field boundaries, or disperse at wider spatial scales [@oberg2007; @zamorano2020, but see @robinson2021].
...
<!-- no effect [@akbar1990; @crawford1998] -->
...
Between-year effects may be important, especially during hot dry years [@kort1988], but this was not considered in our analysis.
...
Finally, about 50\% of growers in western Canada have collected precision agriculture data, and about 85\% have yield monitoring capability [@steel2017], which means that our work has only scratched the surface of what could be (collectively) one of the richest agroecological datasets ever created.
