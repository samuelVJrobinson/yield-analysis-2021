...
- Why no consistent effect for other boundary types? 
  - Our analysis only looks at boundary types, without considering larger landscape context (e.g. Lan's paper)
  - Boundary type itself is not well defined, so perhaps other metrics would be more helpful (height of veg, width of shelterbelt - I think this is important for hedgerows)
  - Ecosystem disservices (e.g. weeds, pest insects) could also be coming from field boundaries, which may negate any effect of ecosystem services; this is still poorly studied (but see Woodcock 2015)
  - Could vary by location; most fields were from central AB, but effects from southern AB may be confounding the signal
  - Additional confounding variables (crop history, differences in tillage)
