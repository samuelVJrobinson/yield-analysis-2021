...
1. Filtered spatial outliers and "inliers" using the procedure from [@vega2019] 
2. Filtered unrealistic yield values (10.75 T/ha for wheat, 8 T/ha for canola and peas), or values less than or equal to zero
3. Filtered extremely low and high outliers (below 1st percentile, above 99th percentile) for: dry yield, steering angle differences, ground speed
4. Filtered points with large changes (>20%) in ground speed
5. Filtered extremely low and high changes in position, indicating that the combine had stopped or moved to another part of the field between points

Due to the large number of data at each field, we sub-sampled filtered data to 50,000 data points per field to reduce computation time.
...
To model this, we fit the following model to each field-year of data:
  ...
  \mu = & Intercept + f(\text{Edge Distance}, b=5)_i + \\
   & f(\text{Easting}, \text{Northing}, b=60)\\
  log(\sigma) =  & Intercept + f(\text{Edge Distance}, b=5)_i + \\
   & f(\text{Easting}, \text{Northing}, b=60)\\
  \end{split}
  \end{equation}
...
The residuals in most of the models displayed both spatial and temporal autocorrelation in the residuals, representing "micro-scale" variation [@cr] that was not captured by the spatial smoothers.

This is extremely common in precision yield data.

We did not include temporal smoothers because spatial and temporal processes are confounded in our model; that is, combining of fields occurs in patterns (e.g. east-to-west) so it is impossible to tell whether lower or higher yields in certain areas of the field were caused by deviations in the yield monitor sensors over time.
Therefore, we assigned all random variation as spatial in nature; customized combine driving patterns may be able to detect temporal variation, but this is beyond the scope of this research.
