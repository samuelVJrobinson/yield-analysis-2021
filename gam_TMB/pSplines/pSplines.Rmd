...
\begin{equation}
\mu = \beta_0 +  f_1(rock) +f_2(litter) + f_3(soil) + f_4(fallPrec) + f_5(sprTmax).\qquad (1)
\label{def:mu}
\end{equation}
...
```c++
    ...
    PARAMETER_VECTOR(log_lambda);// Log penalization parameters (for 5 splines)
...
<details>
  ...
  <summary>Calculate the prior contribution from each of the 5 spline</summary>
  This is where the "complicated stuff" is going on. The following code does two things:
  
  * Retrieve the five $S$ matrices and $\beta$ vectors corresponding to each spline. 
  * Calculate the (multivariate) normal prior on each spline coeff vector $\beta_i$ (with precission matrix $\lambda_i S_i$).
```c++
  int k=0;  // Counter
...
* The contribution from the $i$th spline is $0.5m_i\log(\lambda_i) - 0.5\lambda_i\beta_i^T S_i \beta_i$. A bit of explanation
is needed here. This prior on $\beta$ is a zero mean Gaussian distribution with precission (inverse covariance) matrix
$\lambda_iS_i$. Note that $S_i$  is not full rank, so we are dealing with a degenerate mutivariate distribution.
The part $0.5m_i\log(\lambda_i)$ is comming from the normalizing constant of the multivariate mean Gaussian distribution. A reference is needed here........
...
<details>
  ...
  <summary>Calculate the linear predictor, $\mu$, defined in eqn. (1).   </summary>
```c++
...
<summary>Calculate the negative log likelihood $-\sum_{i=1}^{n}\log P(Y_i|\beta_0,\beta,\sigma)$</summary>
```c++
